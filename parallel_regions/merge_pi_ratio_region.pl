## Classify ROD outlier windows and merge outlier windows into regions
...
# Output the results
open REGION, ">$out.pi_ratio.selected_region";
...
foreach my $chr (sort {$a <=> $b} @chr_array) {
	foreach my $group (sort {$a <=> $b} keys %{$merged{$chr}}) {
		...
		next if @windows < $num;
		my $start = $windows[0] - $window_size/2 + 0.5;
		my $end = $windows[-1] + $window_size/2 - 0.5;
...
if ($f_switch) {
	open OUT, ">$out.pi_ratio_outlier.txt";
	print OUT "CHROM\tPOS\tlg\toutlier\n";
	foreach my $chr (sort {$a <=> $b} @chr_array) {
		foreach my $pos (sort {$a <=> $b} keys %{$rod{$chr}}) {
